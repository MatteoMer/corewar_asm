...
/*   Updated: 2019/01/18 13:31:29 by mmervoye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				init_op_tab(t_asm *asm_h)
{
	...
	asm_h->op[9] = (t_op){"ldi", 3, {T_REG | T_DIR | T_IND, T_DIR \
		| T_REG, T_REG}, 10, 1, 2};
	asm_h->op[10] = (t_op){"sti", 3, {T_REG, T_REG | T_DIR \
		| T_IND, T_DIR | T_REG}, 11, 1, 2};
	asm_h->op[11] = (t_op){"fork", 1, {T_DIR}, 12, 0, 2};
	asm_h->op[12] = (t_op){"lld", 2, {T_DIR | T_IND, T_REG}, 13, 1, 4};
	asm_h->op[13] = (t_op){"lldi", 3, {T_REG | T_DIR | T_IND, T_DIR \
		| T_REG, T_REG}, 14, 1, 2};
